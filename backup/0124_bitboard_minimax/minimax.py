from othellomachine import othellomachine, calculate_valid_moves, board_to_bitboards, bitboards_to_board
import time
...
def search(player_0, player_1, b, depth):
  ...
  if b:
    ...
    for move in valid_cells:
      status, _, new_board, _, _, _ = othellomachine(board, b, move)
      if not status:
        ic(valid_cells)
